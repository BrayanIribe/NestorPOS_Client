...
### El candado del arranque (`bootGate`)

`ensure()` comprueba, rescata si hace falta y **devuelve lo que haya**: el POS abre pase
lo que pase. Es lo correcto para la vigilancia continua y lo incorrecto para el arranque,
porque una caja que abre sin impresión no está "un poco peor" — está emitiendo folios que
nadie se lleva en papel, y eso se descubre al cobrar el primer ticket, con la fila hecha.

`bootGate()` **no devuelve hasta que el servicio de impresión contesta**. Lo llama el POS
al entrar a `/pos`, **en paralelo con la descarga del catálogo** (11-30 MB, minutos en un
enlace lento): ese rato es gratis y es justo el que hace falta para levantar un servicio
...
una caja que no abre nunca, hay tres salidas, y **ninguna es un número de intentos**:

1. La caja que no imprime aquí se declara una vez — paso 2 del asistente, "Vigilancia del
   servicio de impresión: **nunca**". Es la misma llave que ya gobierna la vigilancia.
2. El modo **observación** es una decisión explícita de no tocar nada: el candado avisa y
   sigue, en vez de contradecirla.
3. Fuera de Windows no aplica.

Con el **EMV** sí se rinde, y la diferencia importa: quién decide que no hay con qué
levantarlo es `rescueEmv` (`fatal`), no el candado. Las vías son dos —tarea programada y
ejecutable directo— y comprobarlas por separado aquí dejaba fuera a la caja que arranca
por `emv_direct_launch`. Al rendirse devuelve el motivo tal cual, que ya dice a qué paso
del asistente ir.

Dos cosas que el bucle respeta del daemon en vez de reimplementar: `settleUntil` (se acaba
de lanzar y está arrancando — pedir otro rescate ahí mata el proceso recién nacido) y
...
- Vía de arranque nueva para un servicio (una tarea más, otro ejecutable) → va **sólo**
  dentro de su `rescueX()`. El candado del arranque (`bootGate`) no repite esa decisión a
  propósito: pregunta por `fatal`. Ya pasó una vez — comprobaba `taskExists(EMV_TASK)` por
  su cuenta y, al aparecer `emv_direct_launch`, se saltaba el EMV en las cajas que
  arrancan por ejecutable directo. El merge no lo detecta: no hay conflicto, compila y
  falla en silencio en la caja.
